...
# Check that code which uses PRI* or SCN* macros includes <inttypes.h>
...
for f in $( git ls-files '*.[ch]' |
    ...
    wc="$( < $f grep -E '\b(PRI|SCN)[iu]' | wc -l )"
    if [ "$wc" -ge 1 ]; then
        if ! grep -sqF "<inttypes.h>" $f; then
            echo "$f uses PRI* or SCN* macros but does not include <inttypes.h>"
            fail=1
        fi
    fi
done
