...
# Check that code which uses errno(3) includes <errno.h>
...
for f in $( git ls-files '*.[ch]' | grep -v generator/states ); do
    wc="$( < $f grep -E '\berrno\b' | grep -Ev '/\*.*errno' | wc -l )"
    if [ "$wc" -ge 1 ]; then
        if ! grep -sqF "<errno.h>" $f; then
            echo "$f uses errno(3) but does not include <errno.h>"
            fail=1
        fi
    fi
done
