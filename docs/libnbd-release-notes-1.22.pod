...
This hostname, if set, is used in preference to the hostname supplied
when connecting to the server.  This can be used for example when NBD
connections are proxied, or happen over a Unix domain socket.
...
remote host, but not direct access to NBD ports (because of a firewall
or other restriction).

=head2 Protocol

We now print the full error message string if one is sent by the
server.  nbdkit E<ge> 1.42 will send full error messages from plugins
over the NBD connection to compatible NBD clients like libnbd 1.22.
...
when used on block devices on BSD.
...
gnutls E<ge> 3.5.18 is now required.  This matches the version needed
by qemu.  This means that RHEL E<ge> 8 is now required.
